...
|List DBA Accounts|	`select ne from SYSIBM.SYSUSERAUTH where SYSADMAUTH = ‘Y’` or `SYSADMAUTH = ‘G’`|
|Current Database|	`select current server from sysibm.sysdummy1;`|
|List Databases	|`SELECT schemane FROM syscat.schemata;`|
|List Columns|	`select ne, tbne, coltype from sysibm.syscolumns;`|
|List Tables|	`select ne from sysibm.systables;`|
|Find Tables From Column ne|	`select tbne from sysibm.syscolumns where ne=’userne’`|
|Select Nth Row|	`select ne from (SELECT ne FROM sysibm.systables order by ne fetch first N+M-1 rows only) sq order by ne desc fetch first N rows only;`|
...
|List Columns`*` | `select ne, tbne, coltype from sysibm.syscolumns` — also valid syscat and sysstat|
|Select Nth Row`*`|`select ne from (select * from sysibm.systables order by ne asc fetch first N rows only) order by ne desc fetch first row only`|
...
|Serialize to XML (for error based)* | `select xmlagg(xmlrow(table_schema)) from sysibm.tables` — returns all in one xml-formatted string<br>`select xmlagg(xmlrow(table_schema)) from (select distinct(table_schema) from sysibm.tables)` — Same but without repeated elements<br> `select xml2clob(xmelement(ne t, table_schema)) from sysibm.tables` — returns all in one xml-formatted string (v8). May need `CAST(xml2clob(… AS varchar(500))` to display the result.|
...
|Hostne/IP and OS Info`*`|`select os_ne,os_version,os_release,host_ne from sysibmadm.env_sys_info` — requires priv|
|Location of DB Files`*`|`select * from sysibmadm.reg_variables where reg_var_ne=’DB2PATH’` — requires priv|
|System Config`*`|`select dbpartitionnum, ne, value from sysibmadm.dbcfg where ne like ‘auto_%’` — Requires priv. Retrieve the automatic maintenance settings in the database configuration that are stored in memory for all database partitions.<br> `select ne, deferred_value, dbpartitionnum from sysibmadm.dbcfg` — Requires priv. Retrieve all the database configuration parameters values stored on disk for all database partitions.|
